...
long ngx_pagesize = 4 * 1024;

ngx_radix_tree_t *
ngx_radix_tree_create(ngx_int_t preallocate)
{
    uint32_t           key, mask, inc;
    ngx_radix_tree_t  *tree;

//    tree = ngx_palloc(pool, sizeof(ngx_radix_tree_t));
    tree = malloc(sizeof(ngx_radix_tree_t));
    if (tree == NULL) {
        return NULL;
    }
...
ngx_radix_alloc(ngx_radix_tree_t *tree)
{
    ...
    if (tree->size < sizeof(ngx_radix_node_t)) {
        int err = posix_memalign((void**)&tree->start, ngx_pagesize, ngx_pagesize);
        if (tree->start == NULL) {
            return NULL;
        }
