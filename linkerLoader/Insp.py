...
class PosTransfer:
    ...
    def in_first_half(self, mod):
        return True if mod>self.FIRST_HALF else False
        
    def addr_to_off(self, addr):
        line_number = addr/self.LINELEN
        line_offset = addr%self.LINELEN*3 + self.in_first_half(addr%self.LINELEN)
        print("add[%d] ---> [%d]" % (addr, line_number * self.LINELEN + line_offset))
...
#        print("addr %d get upper bound %d" % (addr, self.lower_bound(addr) + self.BYTES_PER_LINE - 1))
    ...
    def in_same_line (self, addr_start, addr_end):
        print("running %d  %d" %(addr_start, addr_end))
#        True if (addr_end/self.BYTES_PER_LINE == addr_start/self.BYTES_PER_LINE) else False
#        return True
    ...
    def trans(self, range_start, range_end):
        ...
        for r in addr_range :
            ...
            iter1 = self.begin.copy()
            iter2 = self.begin.copy()
...
#        attach的单位很奇怪，不是像素点
        ...
        iter_end.forward_chars(52*3)
        

        
        self.transfer = PosTransfer(self.textbuffer.get_start_iter())
        iters = self.transfer.trans(34, 97)
        iters.append( (iter_start, iter_end) )

#        self.textbuffer.apply_tag(head_tag, iter_start, iter_end)
        iter_start.forward_chars(152*3)
        iter_end.forward_chars(152*3)
#        self.textbuffer.apply_tag(head_tag, iter_start, iter_end)
